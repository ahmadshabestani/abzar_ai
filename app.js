...
function toast(t){const x=$('#toast');if(!x)return;x.textContent=t;x.style.display='block';clearTimeout(window._t);window._t=setTimeout(()=>x.style.display='none',2200)}
...
function speak(text){
  ...
  u.lang='fa-IR';u.rate=.95;u.pitch=1;
  const voices=window.speechSynthesis.getVoices();
  const fa=voices.find(v=>/^fa(-|_)/i.test(v.lang));
  if(fa)u.voice=fa;
...
async function send(text,{speakAnswer=true}={}){
  ...
  try{
    ...
    if(speakAnswer)speak(answer);
    if($('#recommend'))$('#recommend').classList.remove('hidden');
  }catch(e){typing.textContent='متأسفانه ارتباط با موتور هوش مصنوعی برقرار نشد. تنظیمات Backend و کلید API را بررسی کن.';toast(e.message||'خطای ارتباط با AI')}
